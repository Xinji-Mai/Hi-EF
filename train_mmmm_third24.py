...
from model_sirv_third24 import AICLIP
from data_prepro_sirv_third_24 import data_prepro

devices = [3]

mfilename = "sirvthird24"
ckp = None
...
def run(experiment_name: str,
    ...
    model = AICLIP()
    # checkpoint = torch.load(ckp, map_location = 'cpu') # 这里假设您的checkpoint文件名是best-checkpoint.ckpt，您可以根据您的实际文件名来修改
    ...
    logger = TensorBoardLogger('runs', name='second')
    ...
    trainer.fit(model, train_dataloaders=train_loader,
                val_dataloaders=val_loader,ckpt_path='sec_checkpoint/sirvthird.ckpt')
