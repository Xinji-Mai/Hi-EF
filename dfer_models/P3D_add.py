...
class P3D(pl.LightningModule):
    def __init__(self, block, layers, modality='RGB',
        ...
        self.model = baseP3D(Bottleneck, [3, 8, 36, 3], modality=modality)
        # self.conv1 = nn.Conv3d(3, 64, kernel_size=7, stride=(1, 2, 2),
        #                        padding=(3, 3, 3), bias=False)
        self.input_channel = 3 if modality == 'RGB' else 2  # 2 is for flow
        self.cnt = 0
    ...
    def on_test_epoch_end(self) -> None:
        ...
        display_cls_confusion_matrix(self.confusion_matrix, labels_7, test_number_7, name_7,'p3d_add')

        return super().on_test_epoch_end()

    def test_step(self, batch, batch_idx):
        f_frames, o_frames, p_frames, label = self.parse_batch_train(batch)
        loss, preds, war, uar = self.forward(batch)
