...
class baseP3D(nn.Module):
    def __init__(self, block, layers, modality='RGB',
                 shortcut_type='B', num_classes=7, dropout=0.5, ST_struc=('A', 'B', 'C')):
        self.inplanes = 64
        super(baseP3D, self).__init__()
        ...
        self.maxpool_3 = nn.MaxPool3d(kernel_size=(1, 1, 1), padding=0,
                                      stride=(1, 1, 1))  # pooling layer for res 4.
        self.layer1 = self._make_layer(block, 64, layers[0], shortcut_type)
        self.layer2 = self._make_layer(block, 128, layers[1], shortcut_type, stride=2)
        self.layer3 = self._make_layer(block, 256, layers[2], shortcut_type, stride=2)
        self.layer4 = self._make_layer(block, 512, layers[3], shortcut_type, stride=2)

        self.avgpool = nn.AdaptiveAvgPool2d(1)  # pooling layer for res5.
    ...
    def forward(self, f_frames):
        ...
        x = f_frames
        x = x.permute(0, 2, 1, 3, 4)# [8, 3, 16, 112, 112]
...
class P3D(pl.LightningModule):
    def __init__(self, block, layers, modality='RGB',
                 shortcut_type='B', num_classes=7, dropout=0.5, ST_struc=('A', 'B', 'C')):
        self.inplanes = 64
        super(P3D, self).__init__()

        self.model_f = baseP3D(Bottleneck, [3, 8, 36, 3], modality=modality)
        self.model_o = baseP3D(Bottleneck, [3, 8, 36, 3], modality=modality)
        self.model_p = baseP3D(Bottleneck, [3, 8, 36, 3], modality=modality)
        # self.conv1 = nn.Conv3d(3, 64, kernel_size=7, stride=(1, 2, 2),
        #                        padding=(3, 3, 3), bias=False)
        self.input_channel = 3 if modality == 'RGB' else 2  # 2 is for flow
        self.cnt = 0
    ...
    def forward(self, batch):
        ...
        f = feature_1_f + feature_2_f + feature_3_f
        o = feature_1_o + feature_2_o + feature_3_o
        p = feature_1_p + feature_2_p + feature_3_p

        logits = self.fc(self.dropout(f+o+p))
    ...
    def on_test_epoch_end(self) -> None:
        ...
        display_cls_confusion_matrix(self.confusion_matrix, labels_7, test_number_7, name_7,'p3d_add_add')

        return super().on_test_epoch_end()

    def test_step(self, batch, batch_idx):
        f_frames_1, o_frames_1, p_frames_1, f_frames_2, o_frames_2, p_frames_2, f_frames_3, o_frames_3, p_frames_3, daudio_1, dtext_1, daudio_2, dtext_2,daudio_3, dtext_3, label_3, label_4, valid_list_o_1, valid_list_f_1, valid_list_p_1,valid_list_o_2, valid_list_f_2, valid_list_p_2, valid_list_o_3, valid_list_f_3, valid_list_p_3= self.parse_batch_train(batch)
        loss, preds, war, uar = self.forward(batch)
...
if __name__ == '__main__':
    ...
    # device ='cpu'
    model = P3D199(pretrained=False, num_classes=7)
    model.to(device)
    ...
    for i in range(1000):
        ...
        # x = torch.rand(32, 3, 112, 112, 16).to(device)

        data = torch.autograd.Variable(torch.rand(10, 3, 112, 112, 8)).to(device)
