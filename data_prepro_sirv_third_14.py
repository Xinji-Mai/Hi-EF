...
class data_prepro(Dataset):
    def __init__(self, 
        ...
        exlsx = pd.read_csv('/path/to/DFER_Datasets/SIRV_final/train.csv')
        ...
        for idx, row in exlsx.iterrows():
            key = row[0]
            cropface_path = os.path.join(self.f_image_dir, key)
            contents = os.listdir(cropface_path)
            if len(contents) >= 8:
                self.sample[key] = 1
        ...
        for idx, row in mxlsx.iterrows():
            ...
            clip3 = row[3]
            clip4 = row[4]
            if type(self.emo[clip4]) == int:
                if clip3 in self.sample:
    ...
    def __getitem__(self, idx):
        ...
        return f_frames_1, o_frames_1, daudio_1, dtext_1, label_1, label_4, valid_list_o_1, valid_list_f_1
