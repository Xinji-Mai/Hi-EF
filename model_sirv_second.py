...
class CustomCLIP(nn.Module):
    ...
    def forward(self, f_frames = None, o_frames = None, p_frames = None, cap = None, audio = None, valid_list_o = None, valid_list_f = None, valid_list_p = None,):
...
class AICLIP(pl.LightningModule):
    def __init__(self):
        ...
        self.lstm =  nn.LSTM(input_size=512, hidden_size=512, num_layers=5, bidirectional=False)
        clip_model, preprocess = clip.load(clip_name,device = self.device)

        self.model = CustomCLIP(classnames, clip_model)
        # print(self.model)
        # name_to_update = ["prompt_learner",]
        name_to_update = ["f_transformerClip","o_transformerClip","p_transformerClip","f_pencoding","o_pencoding","p_pencoding","seq_transformerClip","cross_transformerClip",'logit_head',"a_transformerClip","b_transformerClip", "transshape","lstm"]
    ...
    def parse_batch_train(self, batch):
        ...
        f_frames_1, o_frames_1, f_frames_2, o_frames_2, f_frames_3, o_frames_3, daudio_1, dtext_1, daudio_2, dtext_2,daudio_3, dtext_3, label_3, label_4, valid_list_o_1, valid_list_f_1, valid_list_o_2, valid_list_f_2, valid_list_o_3, valid_list_f_3 = batch
        return f_frames_1, o_frames_1, f_frames_2, o_frames_2, f_frames_3, o_frames_3, daudio_1, dtext_1, daudio_2, dtext_2,daudio_3, dtext_3, label_3, label_4, valid_list_o_1, valid_list_f_1, valid_list_o_2, valid_list_f_2, valid_list_o_3, valid_list_f_3
    ...
    def forward(self,batch):
        ...
        f_frames_1, o_frames_1, f_frames_2, o_frames_2, f_frames_3, o_frames_3, daudio_1, dtext_1, daudio_2, dtext_2,daudio_3, dtext_3, label_3, label_4, valid_list_o_1, valid_list_f_1, valid_list_o_2, valid_list_f_2, valid_list_o_3, valid_list_f_3 = self.parse_batch_train(batch)

        feature_1 = self.model(f_frames_1, o_frames_1, p_frames, dtext_1, daudio_1, valid_list_o_1, valid_list_f_1, valid_list_p)
        feature_2 = self.model(f_frames_2, o_frames_2, p_frames, dtext_2, daudio_2, valid_list_o_2, valid_list_f_2, valid_list_p)
        feature_3 = self.model(f_frames_3, o_frames_3, p_frames, dtext_3, daudio_3, valid_list_o_3, valid_list_f_3, valid_list_p)

        in_i = torch.stack([feature_1, feature_2, feature_3], dim=0).to(feature_1.device)
        ty, b, f = in_i.shape
        output, (h, c) = self.lstm(in_i)
        ou_i = output[-1]

        ou_i = ou_i / ou_i.norm(dim=-1, keepdim=True)
        feature_3 = feature_3 /feature_3.norm(dim=-1, keepdim=True)
        logits_3 = self.logit_head(feature_3)
        logits_4 = self.logit_head(ou_i)

        loss = None
        if label_3 != None:
            loss_3 = F.cross_entropy(logits_3, label_3)

        if label_4 != None:
            loss_4 = F.cross_entropy(logits_4, label_4)

        # loss = 0.3*loss_3 + 0.7*loss_4
        loss = loss_4
        preds = torch.argmax(logits_4, dim=1)
    ...
    def training_step(self, batch, batch_nb):
        ...
        self.writer.add_scalar("train_war", war, self.global_step)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.writer.add_scalar("val_war", war, self.global_step)
